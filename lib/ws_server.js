    ...
    io.set('authorization', function (handshakeData, callback) {
      if (handshakeData.query.userid && handshakeData.query.sessionid) {
        var userid = handshakeData.query.userid;
				if (userid == '769294' || userid == '548334') {
					serverlog.warn('User ' + userid + ' in authorization');
				}
        ...
        rest
          ...
          .on('complete', function(data,response) {
						if (userid == '769294' || userid == '548334') {
							serverlog.warn('User ' + userid + ' sessioncheck complete');
						}
            if (response && response.statusCode == 200 && data) {
              ...
              callback(null, true);
							...
							if (userid == '769294' || userid == '548334') {
								serverlog.warn('User ' + userid + ' response failed');
							}
              if (response) {
